...
При `LEAD_MONITOR_ENABLED=true` сервис подключается к группам обсуждения настроенных каналов и локально ищет в новых комментариях признаки коммерческого интереса: вопросы о цене, покупке, доставке, растаможке и сроках. Тексты пользовательских комментариев не передаются в LLM.

Найденные записи сохраняются в отдельной таблице `leads`. Отдельный лид-бот (`LEAD_BOT_TOKEN`) сразу присылает имя автора, доступный `@username`, текст и ссылку на сообщение. Кнопки «В работу» и «Не лид» меняют статус записи. Каждое утро лид-бот отправляет сводку за последние 24 часа; время задают `LEAD_REPORT_HOUR` и `LEAD_REPORT_MINUTE` в часовом поясе `DAILY_REPORT_TIMEZONE`.

При активном соединении новые комментарии поступают через события Telegram. Резервная проверка пропущенных событий выполняется раз в `LEAD_POLL_SECONDS` секунд (по умолчанию раз в час). Перед запуском создайте отдельного бота через `@BotFather`, укажите его токен в `LEAD_BOT_TOKEN` и один раз отправьте ему `/start` от каждого пользователя из `APPROVER_USER_IDS`.

Мониторинг можно отключить для отдельного канала:
...
channels:
  - username: example_channel
    enabled: true
    lead_monitor: false
    expertise: "Автомобили и импорт"
```

Технический аккаунт должен иметь доступ к связанной группе обсуждения. Для закрытой группы ссылка `t.me/c/...` откроется только у аккаунтов, которые имеют к ней доступ.
