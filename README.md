...
Посмотреть ID всех каналов и чатов, доступных пользовательскому аккаунту:

~~~bash
docker compose run --rm commenter python -m app.list_dialogs
~~~

Для приватного канала или супергруппы скопируйте значение PEER_ID целиком,
включая префикс -100. Обычные приватные группы имеют отрицательный ID без -100.
