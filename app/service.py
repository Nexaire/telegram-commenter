...
def notification_text(title: str, text: str, url: str | None) -> str:
    ...
    prefix = (
        f"🎸 <b>Найдена заявка на кавер-группу</b>\n\n<b>Источник:</b> {safe_title}\n\n"
    )
...
class CoverMonitorService:
    def __init__(self, settings, db, workflow_bot):
    ...
    async def publisher_loop(self):
        while True:
            ...
            rows = await self.db.all(
                "SELECT * FROM matches WHERE delivery_status IN ('pending','error') "
    ...
    async def publish_match(self, match: dict):
        ...
        if self.settings.dry_run:
            await self.db.execute(
                "UPDATE matches SET delivery_status='dry_run' WHERE id=?",
                (match_id,),
            )
            log.info(
                "match_dry_run",
            ...
            return
        try:
            sent = await self.workflow_bot.publish_match(match, body)
            await self.db.execute(
                "UPDATE matches SET delivery_status='published', "
            ...
            await self.db.execute(
                "UPDATE matches SET delivery_status='error', attempt_count=?, "
